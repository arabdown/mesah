...
import 'package:listar_flutter_pro/widgets/app_upload_image.dart';
...
class _SubmitState extends State<Submit> {
  ...
  void _onSelectState() async {
    if (_country == null) {
      AppBloc.messageBloc.add(
        OnMessage(message: Translate.of(context).translate('choose_country')),
      );
      return;
    }
    final selected = await Navigator.pushNamed(
  ...
  void _onSelectCity() async {
    if (_state == null) {
      AppBloc.messageBloc.add(
        OnMessage(message: Translate.of(context).translate('choose_state')),
      );
      return;
    }
    final selected = await Navigator.pushNamed(
  ...
  bool _validData() {
    ...
    if (_featureImage == null) {
      AppBloc.messageBloc.add(OnMessage(message: 'feature_image_require'));
      return false;
    }

    ///Category
    if (_categories.isEmpty) {
      AppBloc.messageBloc.add(OnMessage(message: 'category_require'));
      return false;
    }

    ///Facilities
    if (_facilities.isEmpty) {
      AppBloc.messageBloc.add(OnMessage(message: 'facilities_require'));
      return false;
    }

    ///Country
    if (_country == null) {
      AppBloc.messageBloc.add(OnMessage(message: 'country_require'));
      return false;
    }
